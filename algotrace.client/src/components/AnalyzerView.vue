...
const algorithmGuides: Record<string, { title: string, description: string, howItWorks: string[], pros: string[], cons: string[] }> = {
  'levenshtein': {
    title: 'Відстань Левенштейна',
    description: 'Гнучкий посимвольний аналіз. На відміну від суворого порядкового порівняння, цей метод оцінює "мінімальну ціну" (кількість редагувань) для перетворення одного рядка в інший. Ідеальний для пошуку частково зміненого коду.',
    howItWorks: [
      'Код розбивається на рядки, які нормалізуються (видаляються зайві пробіли та ігнорується регістр).',
      'Алгоритм бере кожен рядок з першого файлу та порівнює його з рядками другого.',
      'Математично обчислюється кількість операцій: скільки символів треба вставити, видалити або замінити (наприклад, "var index" -> "var count_idx" = кілька замін).',
      'Якщо розрахована відстань менша за певний відсоток від довжини рядка (зазвичай різниця < 15%), рядки позначаються як модифіковані клони.'
    ],
    pros: [
      'Чудово виявляє незначні зміни: перейменування змінних, зміну типу даних, додавання модифікаторів.',
      'Не вимагає точного збігу, тому ловить "розумний" Copy-Paste на рівні окремих інструкцій.'
    ],
    cons: [
      'Дуже ресурсомісткий (O(N*M)) для великих файлів, оскільки порівнює кожен рядок з кожним.',
      'Не враховує перестановку слів місцями в межах одного рядка (наприклад, зміну порядку параметрів).'
    ]
  },
  'line_matching': {
    title: 'Порядкове Порівняння',
    description: 'Базовий і найсуворіший алгоритм текстового порівняння. Шукає виключно ідентичні (точні) збіги великих блоків коду, лінійно скануючи файли зверху вниз.',
    howItWorks: [
      'Файли розбиваються на рядки, з яких видаляються пробіли та коментарі (залежно від налаштувань).',
      'Алгоритм шукає точні збіги рядків між файлами.',
      'Коли знайдено збіг, алгоритм перевіряє наступні рядки. Якщо вони також збігаються, формується "блок".',
      'Одиночні збіги (наприклад, закриваючі дужки "}") відкидаються, щоб уникнути хибних спрацьовувань. Фіксуються лише блоки від 3-4 рядків.'
    ],
    pros: [
      'Надзвичайно швидкий та легковаговий метод.',
      'Гарантовано виявляє нахабний прямий плагіат без жодних змін тексту.'
    ],
    cons: [
      'Критично вразливий до будь-яких втручань: зміна хоча б одного символу або додавання порожнього рядка всередині блоку "розриває" збіг.',
      'Не виявляє перейменування змінних або зміну форматування.'
    ]
  },
  'rabin_karp': {
    title: 'Алгоритм Рабіна-Карпа',
    description: 'Алгоритм на базі "ковзного вікна". На відміну від порядкового порівняння, він миттєво знаходить ідентичні блоки коду навіть якщо вони були переміщені в абсолютно інше місце файлу.',
    howItWorks: [
      'Нормалізований текст ділиться на уявні "вікна" певного розміру (наприклад, 5 рядків).',
      'Для тексту в першому вікні обчислюється унікальне числове хеш-значення.',
      'Вікно зсувається на 1 рядок вниз. Завдяки кільцевому хешуванню (rolling hash), новий хеш обчислюється миттєво на базі попереднього.',
      'Замість порівняння довгого тексту, алгоритм перевіряє лише числа (хеші). Якщо хеші збігаються, перевіряється текст (для виключення колізій), і фіксується знайдений плагіат.'
    ],
    pros: [
      'Надзвичайно ефективний алгоритм для пошуку скопійованих методів, які перенесли в інші класи або файли.',
      'Має високу швидкодію (O(N+M)) навіть на проектах із сотнями файлів.'
    ],
    cons: [
      'Як і порядкове порівняння, шукає лише точні збіги в межах "вікна". Зміна одного символу повністю змінює хеш.',
      'Чутливий до обраного розміру вікна (надто велике — пропускає клони, надто мале — дає хибні збіги).'
    ]
  },
  'ngram_search': {
    title: 'Пошук за N-грамами',
    description: 'Метод, що аналізує код як "мішок слів". Дозволяє виявити плагіат, у якому рядки поміняли місцями або "розбавили" новим кодом. Покриває недоліки Рабіна-Карпа.',
    howItWorks: [
      'Код розбивається на перекривні послідовності з N елементів (символів або слів).',
      'Для обраного блоку (наприклад, методу) формується математична множина всіх його N-грамів.',
      'Алгоритм порівнює такі множини між двома програмами, використовуючи індекс Джаккарда (відношення кількості спільних N-грамів до загальної кількості унікальних).',
      'Якщо перетин множин перевищує певний поріг (наприклад, 70%), блоки вважаються схожими, незважаючи на змінений порядок.'
    ],
    pros: [
      'Високо стійкий до перестановки незалежних рядків місцями.',
      'Стійкий до вставки дрібного "сміттєвого" коду з метою обману антиплагіату.'
    ],
    cons: [
      'Вимагає більше оперативної пам\'яті для генерації та зберігання великої кількості фрагментів.',
      'Може давати хибні спрацьовування на дуже стандартизованому boilerplate-коді.'
    ]
  },
  'jaccard_token': {
    title: 'Токени Джаккарда',
    description: 'Оцінює загальну лексичну схожість. Цей метод взагалі ігнорує структуру чи порядок виконання програми, порівнюючи лише унікальний "словник" розробника.',
    howItWorks: [
      'Спеціальний лексер (аналізатор) перетворює весь текст програми на потік токенів (ключові слова, змінні, оператори).',
      'З потоку видаляються всі повтори, утворюючи унікальні множини використовуваних токенів для Файлу 1 та Файлу 2.',
      'Обчислюється індекс Джаккарда: кількість спільних токенів ділиться на загальну кількість унікальних токенів обох файлів.'
    ],
    pros: [
      'Абсолютно незалежний від структури програми, порядку методів та логіки виконання.',
      'Миттєво показує, чи використовують автори однаковий набір ідентифікаторів та бібліотек.'
    ],
    cons: [
      'Сліпий до логіки: "a = b - c" і "b = c - a" будуть сприйняті як ідентичні, оскільки набір токенів однаковий.',
      'Використовується здебільшого як допоміжний метод для глобальної оцінки.'
    ]
  },
  'winnowing': {
    title: 'Вінновінг (Winnowing)',
    description: 'Промисловий стандарт антиплагіату (лежить в основі відомої системи MOSS від Стенфорда). Генерує унікальні "відбитки пальців" коду, що дозволяє знаходити вкрадені шматки навіть після значних маніпуляцій.',
    howItWorks: [
      'Код очищується і перетворюється на суцільний потік абстрактних токенів (наприклад, всі змінні замінюються на ID).',
      'З токенів формуються k-грами (послідовності токенів), кожна з яких хешується.',
      'Алгоритм проходить по хешах ковзним вікном розміру W. З кожного вікна вибирається лише один, найменший хеш — він стає "відбитком пальця" документа.',
      'Відбитки обох документів порівнюються. Спільні відбитки точно вказують на ідентичні логічні послідовності.'
    ],
    pros: [
      'Еталонна стійкість до перейменування змінних, зміни форматування та додавання коментарів.',
      'Гарантує математичне знаходження всіх збігів, довших за визначений розробником поріг.'
    ],
    cons: [
      'Може пропускати дуже дрібні фрагменти коду (менші за розмір вікна W).',
      'Не виявляє семантичних змін (якщо плагіатор переписав цикл "for" на цикл "while").'
    ]
  },
  'ast_hashing': {
    title: 'Хешування AST',
    description: 'Найшвидший алгоритм у категорії структурного аналізу (AST). Створює унікальний "паспорт" (хеш) для кожного логічного блоку програми, забезпечуючи O(1) пошук збігів.',
    howItWorks: [
      'Код розбирається на Абстрактне Синтаксичне Дерево (AST) — ієрархію його структури (If, For, Assignment тощо).',
      'Метод обходить дерево від листя до кореня. Для кожного вузла обчислюється хеш на основі його типу та хешів його дітей.',
      'Ці хеші записуються у швидку хеш-таблицю.',
      'Алгоритм просто шукає однакові хеші у двох таблицях. Якщо хеші збіглися — значить, десь існують абсолютно однакові логічні піддерева (наприклад, повністю однакові функції).'
    ],
    pros: [
      'Надзвичайно швидкий пошук ідентичних структур у дереві.',
      'Стійкий до переміщення коду. Якщо функцію вкрали і перенесли в інший файл без змін — її хеш залишиться тим самим і буде миттєво знайдений.'
    ],
    cons: [
      'Хеш жорстко залежить від структури. Додавання одного порожнього оператора (сміття) змінить хеш усього дерева та сховає плагіат.',
      'Часто буває чутливий до перейменування змінних (залежно від налаштувань парсера).'
    ]
  },
  'ast_compare': {
    title: 'Пряме Порівняння AST',
    description: 'Класичний алгоритм "накладання" дерев. На відміну від хешування, він паралельно обходить обидва AST зверху вниз, виявляючи загальну архітектурну схожість програм.',
    howItWorks: [
      'Після генерації AST для обох файлів алгоритм бере кореневі вузли (початок програм).',
      'Відбувається рекурсивне паралельне проходження обох дерев.',
      'На кожному кроці порівнюються типи вузлів та кількість їхніх нащадків.',
      'Якщо структури розходяться, алгоритм відмічає розбіжність, але продовжує порівнювати інші гілки дерева. Обчислюється відсоток вузлів, що структурно збіглися.'
    ],
    pros: [
      'Абсолютно стійкий до будь-яких візуальних змін: кількості пробілів, переносів рядків, коментарів (бо вони відсутні в AST).',
      'Дає комплексну оцінку схожості архітектури всього файлу.'
    ],
    cons: [
      'Вразливий до переміщення блоків коду. Якщо перша програма має функції [A, B], а друга [B, A], послідовне порівняння зверху вниз дасть збій.',
      'Рекурсія може бути ресурсомісткою для дуже глибоко вкладених структур.'
    ]
  },
  'subtree_isomorphism': {
    title: 'Ізоморфізм Піддерев',
    description: 'Ультимативний метод пошуку в AST. Вирішує головну проблему "Прямого порівняння" — він "вириває" логічні блоки та перевіряє їх на ізоморфність (топологічний збіг) у будь-якому місці іншого файлу.',
    howItWorks: [
      'З AST першого файлу витягуються лише "значущі" піддерева (наприклад, ті, що відповідають за тіла циклів, If-блоків або цілих методів).',
      'Алгоритм бере таке піддерево і намагається накласти його на всі можливі піддерева другої програми.',
      'Перевіряється строгий структурний ізоморфізм — чи відповідають структури одна одній безвідносно до тексту в них.',
      'Якщо знайдено ізоморфізм, фіксується високоймовірний факт "Copy-Paste" конкретної логіки.'
    ],
    pros: [
      'Ідеально розпізнає плагіат, коли код був повністю переформатований, перейменований і засунутий усередину інших методів чи класів.',
      'Працює на рівні "форми" логіки, ігноруючи її текстовий зміст.'
    ],
    cons: [
      'Висока обчислювальна складність: намагатися накласти кожен вузол на кожен дуже дорого.',
      'Може давати хибні спрацьовування на типових, тривіальних блоках (наприклад, базовий цикл for від 0 до N).'
    ]
  },
  'cfg': {
    title: 'Граф Потоку Керування (CFG)',
    description: 'Переводить аналіз у площину графів. Аналізує безпосередньо "шляхи" виконання програми: розгалуження (if, switch) та цикли, утворюючи Граф Потоку Керування.',
    howItWorks: [
      'Код розбивається на базові блоки (ділянки без розгалужень).',
      'Будується граф (CFG), де вузли — це блоки, а ребра (стрілки) — це переходи між ними (логіка управління).',
      'Алгоритм використовує динамічне програмування (або зіставлення підграфів), щоб знайти в обох програмах найдовші ідентичні ланцюжки виконання.',
      'Знайдений збіг означає, що програми приймають ідентичні рішення у такому ж порядку.'
    ],
    pros: [
      'Ловить плагіат, навіть якщо студент переставив незалежні рядки місцями або змінив цикли (наприклад, виніс ініціалізацію перед циклом).',
      'Стійкий до додавання "сміттєвих" обчислень, які не створюють нових розгалужень.'
    ],
    cons: [
      'Надзвичайно важко побудувати точний CFG для мов з динамічною типізацією.',
      'Якщо повністю змінити керуючу структуру (наприклад, замінити цикл на рекурсію), граф CFG повністю зміниться, обдуривши алгоритм.'
    ]
  },
  'pdg': {
    title: 'Граф Залежностей Даних (PDG)',
    description: 'Еволюція графового аналізу (найскладніший і найпотужніший метод). Відстежує, звідки змінні беруть дані та де вони використовуються ("data flow").',
    howItWorks: [
      'Будується граф (PDG), де вузли — це операції з даними, а ребра — потоки залежностей (наприклад, "змінна Х ініціалізована тут, а прочитана аж ось там").',
      'Алгоритм ігнорує керуючі структури і шукає виключно збіги ребер залежностей між двома графами.',
      'Якщо в обох програмах "Дані А впливають на Обчислення Б" однаковим шляхом, фіксується плагіат логіки.'
    ],
    pros: [
      'Невразливий майже до жодної обфускації. Він ловить саму "суть" алгоритму (семантику), навіть якщо код переписано до невпізнання.',
      'Стійкий до заміни керуючих структур (циклів, умов), бо орієнтується на рух даних.'
    ],
    cons: [
      'Аналіз потоків даних (Data Flow Analysis) надзвичайно ресурсомісткий. Порівняння PDG є NP-повною задачею.',
      'Складний для реалізації універсального парсера, що підтримує багато мов.'
    ]
  },
  'subgraph_isomorphism': {
    title: 'Ізоморфізм Підграфів',
    description: 'Абсолютно строгий математичний інструмент порівняння графів (як CFG, так і PDG). Шукає точне взаємно однозначне "вкладання" одного графа в інший.',
    howItWorks: [
      'Обидві програми перетворюються на графи залежностей.',
      'Запускається алгоритм з перебором (backtracking) та евристиками.',
      'Він перевіряє: чи можна взяти граф (або його частину) з Програми А і математично ідеально накласти на Програму Б так, щоб збереглися всі вершини та всі напрямки зв\'язків (ребер).'
    ],
    pros: [
      'Дає 100% математичне підтвердження, що логіка програм ідентична на структурному рівні.',
      'Відсутні хибні спрацьовування на великих ізоморфних підграфах.'
    ],
    cons: [
      'Задача є NP-повною (експоненційна складність часу виконання). Щоб система не зависла, встановлюється ліміт ітерацій.',
      'Занадто строгий: мінімальна зміна логіки зламає математичний ізоморфізм.'
    ]
  },
  'halstead': {
    title: 'Метрики Холстеда',
    description: 'Класичні лінгвістичні метрики складності коду. Вони оцінюють код не як структуру, а як сукупність слів та дій.',
    howItWorks: [
      'Код парситься, і система підраховує чотири базові показники: унікальні оператори (if, +, =), унікальні операнди (змінні, константи), та їх загальну кількість.',
      'За математичними формулами Моріса Холстеда обчислюються: Словник, Об\'єм програми, Складність та Зусилля для написання.',
      'Дві програми порівнюються за цими метриками. Аномально схожі цифри (відхилення < 5%) можуть свідчити про однаковий підхід до вирішення задачі.'
    ],
    pros: [
      'Слабо залежить від мови програмування та форматування.',
      'Дозволяє швидко оцінити загальний рівень складності програм (наприклад, чи не використав студент надто складне рішення для простої задачі).'
    ],
    cons: [
      'Це глобальні метрики, вони вказують лише на ймовірність плагіату. Різні алгоритми випадково можуть мати схожі показники.',
      'Не показує конкретні місця в коді, де відбулося копіювання.'
    ]
  },
  'mccabe': {
    title: 'Складність Маккейба (Цикломатична)',
    description: 'Метрика, яка вимірює рівень заплутаності та кількості ухвалених рішень (розгалужень) у програмі.',
    howItWorks: [
      'Аналізатор проходить по коду і рахує всі точки розгалуження керування: if, while, for, case, логічні оператори AND/OR.',
      'За формулою Маккейба обчислюється число (цикломатична складність = кількість розгалужень + 1).',
      'Якщо два великі файли мають абсолютно ідентичну цикломатичну складність, це означає, що вони використовують однакову структуру прийняття рішень (однакова кількість if/else).'
    ],
    pros: [
      'Швидкий і дуже простий спосіб оцінити заплутаність коду.',
      'Ідеальний індикатор для виявлення студентів, які переписали код, зберігши повністю ідентичну логіку if-else гілок.'
    ],
    cons: [
      'Як і Холстед, вимагає підтвердження іншими методами (наприклад, CFG аналізом), оскільки не дає стовідсоткової впевненості.',
      'Не реагує на обсяг лінійного коду (програма з тисячі присвоювань без умов матиме таку ж складність (1), як і програма з одного рядка).'
    ]
  }
};
...
const currentGuide = computed(() => {
  ...
  return algorithmGuides[activeAlgorithm.value.method] || null;
});
</script>
...
<template>
  <div class="min-vh-100 dashboard-bg pb-5" v-if="report">
    ...
    <div v-if="activeAlgorithm && currentGuide" 
         class="guide-trigger" 
         @click="isGuideOpen = true" 
         :class="{ 'hidden': isGuideOpen }">
      <i class="bi bi-journal-bookmark-fill mb-2 fs-5"></i>
      <div class="guide-trigger-text">ДОВІДНИК</div>
    </div>

    <div class="guide-drawer-backdrop" v-if="isGuideOpen" @click="isGuideOpen = false"></div>
    <div class="guide-drawer" :class="{ 'open': isGuideOpen, 'bg-dark text-light': isDarkMode, 'bg-white': !isDarkMode }">
      <div class="p-4 border-bottom d-flex justify-content-between align-items-center" :class="isDarkMode ? 'border-secondary' : 'border-light'">
        <h4 class="fw-black m-0 d-flex align-items-center">
          <i class="bi bi-info-square-fill text-primary me-3"></i> Аналітичний довідник
        </h4>
        <button type="button" class="btn-close" :class="{'btn-close-white': isDarkMode}" @click="isGuideOpen = false"></button>
      </div>
      <div class="p-4 flex-grow-1 overflow-auto custom-scrollbar" v-if="currentGuide">
        <h3 class="fw-bold mb-3 d-flex align-items-center text-primary">
          <i class="bi bi-cpu me-2"></i> {{ currentGuide.title }}
        </h3>
        <p class="fw-medium lh-lg mb-4" :class="isDarkMode ? 'text-light opacity-75' : 'text-secondary'">{{ currentGuide.description }}</p>
        
        <h5 class="fw-bold mt-4 mb-3 d-flex align-items-center"><i class="bi bi-gear-wide-connected text-warning me-2"></i> Принцип роботи</h5>
        <div class="d-flex flex-column gap-3 mb-5">
          <div class="p-3 rounded-3 d-flex align-items-start border shadow-sm" :class="isDarkMode ? 'bg-black bg-opacity-25 border-secondary' : 'bg-light'" v-for="(step, i) in currentGuide.howItWorks" :key="i">
            <span class="badge bg-primary rounded-circle me-3 mt-1 shadow-sm px-2 py-1" style="font-size: 0.85rem;">{{ i + 1 }}</span> 
            <span class="fw-medium" style="font-size: 0.95rem;">{{ step }}</span>
          </div>
        </div>

<div class="row g-4">
          <div class="col-12">
            <div class="p-4 pe-5 rounded-4 border position-relative overflow-hidden" :class="isDarkMode ? 'bg-success bg-opacity-10 border-success border-opacity-25' : 'bg-success bg-opacity-10 border-success border-opacity-25'">
              <div class="position-absolute top-0 end-0 p-3 opacity-10 z-0">
                <i class="bi bi-check-circle-fill fs-1 text-success"></i>
              </div>
              <h6 class="text-success fw-bold text-uppercase tracking-wider mb-3 position-relative z-1"><i class="bi bi-plus-circle-fill me-2"></i> Переваги</h6>
              <ul class="mb-0 small ps-3 fw-medium position-relative z-1" :class="isDarkMode ? 'text-light' : 'text-dark'">
                <li v-for="(pro, i) in currentGuide.pros" :key="i" class="mb-2">{{ pro }}</li>
              </ul>
            </div>
          </div>
          <div class="col-12">
            <div class="p-4 pe-5 rounded-4 border position-relative overflow-hidden" :class="isDarkMode ? 'bg-danger bg-opacity-10 border-danger border-opacity-25' : 'bg-danger bg-opacity-10 border-danger border-opacity-25'">
              <div class="position-absolute top-0 end-0 p-3 opacity-10 z-0">
                <i class="bi bi-exclamation-triangle-fill fs-1 text-danger"></i>
              </div>
              <h6 class="text-danger fw-bold text-uppercase tracking-wider mb-3 position-relative z-1"><i class="bi bi-dash-circle-fill me-2"></i> Обмеження</h6>
              <ul class="mb-0 small ps-3 fw-medium position-relative z-1" :class="isDarkMode ? 'text-light' : 'text-dark'">
                <li v-for="(con, i) in currentGuide.cons" :key="i" class="mb-2">{{ con }}</li>
              </ul>
            </div>
          </div>
        </div>
...
.guide-trigger {
  ...
  cursor: pointer;
  z-index: 1030;
  box-shadow: -4px 0 20px rgba(13, 110, 253, 0.4);
