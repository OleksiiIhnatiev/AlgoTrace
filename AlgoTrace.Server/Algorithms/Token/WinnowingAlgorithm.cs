...
namespace AlgoTrace.Server.Algorithms
{
    public class WinnowingAlgorithm : ITokenAlgorithm
    {
        ...
        public List<DetailedMatch> Execute(List<TokenInfo> sourceTokens, List<TokenInfo> targetTokens, out double similarityScore)
        {
            ...
            var commonHashes = sHashes.Select(h => h.Hash).Intersect(tHashes.Select(h => h.Hash)).ToHashSet();
            ...
            if (commonHashes.Any())
            {
                ...
                matches.Add(new DetailedMatch
                {
                    ...
                    LeftLines = new List<int> { matchedSource.First().StartLine, matchedSource.Last().EndLine },
                    RightLines = new List<int> { matchedTarget.First().StartLine, matchedTarget.Last().EndLine }
                });
            }
            return matches;
        }
        ...
        private List<Fingerprint> GetFingerprints(List<TokenInfo> tokens)
        {
            ...
            for (int i = 0; i <= tokens.Count - K; i++)
            {
                ...
                result.Add(new Fingerprint(gram.GetHashCode(), window.First().Line, window.Last().Line));
            }
            return result;
        }
    }
