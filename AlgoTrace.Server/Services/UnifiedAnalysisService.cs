...
namespace AlgoTrace.Server.Services
{
    public class UnifiedAnalysisService : IUnifiedAnalysisService
    {
        ...
        public UnifiedAnalysisResponse Analyze(UnifiedAnalysisRequest request)
        {
            ...
            {
                var result = ProcessGraph(
                    contentA,
                    contentB,
                    request.Language,
                    categories.GraphBased,
                    parameters
                );
                response.CategoriesResults.Add(result);
            }
                ...
                foreach (var m in matches)
                {
                    ...
                    evidence.MatchedBlocks.Add(
                        new
                        {
                            ...
                            start_line_b = startB,
                            end_line_b = endB,
                            content_a = startA > 0 && startA <= sLinesA.Length
                                ? string.Join(
                                    "\n",
                                    sLinesA.Skip(startA - 1).Take(Math.Max(1, endA - startA + 1))
                                )
                                : "",
                            content_b = startB > 0 && startB <= sLinesB.Length
                                ? string.Join(
                                    "\n",
        ...
        private CategoryResult ProcessTree(
            string a,
            string b,
            string lang,
            List<string> methods,
                    ...
                    categoryResult.Algorithms.Add(
                        new AlgorithmResult
                        {
                            ...
                            Evidence = new
                            {
                                graph_a = new
                                {
                                    nodes = graphA.Nodes.Select(n => new
                                    {
                                        id = n.Id,
                                        line = n.LineIndex + 1,
                                        content = n.Content,
                                    ...
                                    edges = graphA.Edges.Select(e => new
                                    {
                                        source = e.SourceId,
                                        target = e.TargetId,
                                        type = e.Type,
                                ...
                                graph_b = new
                                {
                                    nodes = graphB.Nodes.Select(n => new
                                    {
                                        id = n.Id,
                                        line = n.LineIndex + 1,
                                        content = n.Content,
                                    ...
                                    edges = graphB.Edges.Select(e => new
                                    {
                                        source = e.SourceId,
                                        target = e.TargetId,
                                        type = e.Type,
        ...
        )
        {
            ...
            foreach (var algo in _metricAlgos)
            {
                ...
                if (algo.Key == "halstead")
                {
                    ...
                    {
                        { "halstead_volume", hA.Volume },
                        { "halstead_difficulty", hA.Difficulty },
                    };
                    evidence.MetricsB = new Dictionary<string, double>
                    {
                        { "halstead_volume", hB.Volume },
                        { "halstead_difficulty", hB.Difficulty },
                    };
                }
                else if (algo.Key == "mccabe")
                {
                    ...
                    evidence.MetricsA = new Dictionary<string, double>
                    {
                        { "cyclomatic_complexity", cA },
                    };
                    evidence.MetricsB = new Dictionary<string, double>
                    {
                        { "cyclomatic_complexity", cB },
                    };
                }
